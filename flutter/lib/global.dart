const BASEURL = 'http://122.112.175.64';


const List<String> magicType = ['通常', '装备', '速攻', '永续', '场地', '仪式'];
const List<String> trapType = ['通常', '永续', '反击'];

const List<String> monsterAttr = ['光', '暗', '炎', '水', '地', '风', '神'];
const List<String> monsterType = [
  '通常', '效果', '仪式', '融合', '同调', 'XYZ', '卡通',
  '同盟', '灵魂', '调整', '二重', '灵摆', '反转', '特召',
  '连接'
];

const List<String> monsterRace = [
  '水', '兽', '兽战士', '创造神', '恐龙', '幻神兽', '龙',
  '天使', '恶魔', '鱼', '昆虫', '机械', '植物', '念动力',
  '炎', '爬虫类', '岩石', '海龙', '魔法师', '雷', '战士',
  '鸟兽', '不死', '幻龙', '电子界'
];

const List<String> linkArrow = [
  '↖', '↑', '↗',
  '←', '', '→',
  '↙', '↓', '↘'
];